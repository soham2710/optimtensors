# Test Results & Performance Report - safe-optim-checkpoint v1
...
1. **Adam vs. AdamW Auto-Inference**: Modified `infer_optimizer_type` to raise a `UserWarning` during auto-inference (explaining that Adam and AdamW share identical key structures and auto-inference is best-effort fallback). The loader now accepts an explicit `optimizer_type` parameter as the source of truth.
   - **Verification Test**: `test_adam_adamw_classification_regression` in [test_serde.py](file:///mnt/d/HP%20Shared/Data%20Science/Research/safe-optim-checkpoint/tests/test_serde.py#L298).
...
- *Verification Test*: Defined in [test_architectures.py](file:///mnt/d/HP%20Shared/Data%20Science/Research/safe-optim-checkpoint/tests/test_architectures.py). All checks verified absolute value equality on CPU and GPU.
...
- *Verification Test*: `test_amp_fp16_fp32` and `test_amp_bf16_fp32_with_int16_collision` in [test_amp.py](file:///mnt/d/HP%20Shared/Data%20Science/Research/safe-optim-checkpoint/tests/test_amp.py) (All Pass).
...
- *Verification Test*: `test_memory_leaks` in [test_leaks.py](file:///mnt/d/HP%20Shared/Data%20Science/Research/safe-optim-checkpoint/tests/test_leaks.py) (Pass).
...
- *Verification Test*: `test_concurrent_reads` in [test_concurrency.py](file:///mnt/d/HP%20Shared/Data%20Science/Research/safe-optim-checkpoint/tests/test_concurrency.py) (Pass).
...
- *Verification Test*: `test_adversarial_shape_offset_fuzz` in [test_fuzz.py](file:///mnt/d/HP%20Shared/Data%20Science/Research/safe-optim-checkpoint/tests/test_fuzz.py#L375) (Pass).
